...
from app.incomes.service import add_income, delete_income, get_all_incomes
...
def add_income_route():
    ...
    result, status_code = add_income(title, description, price, user_id)
...
def delete_income_route(income_id):
    user_id = get_jwt_identity()
    result, status_code = delete_income(income_id, user_id)
    return jsonify(result), status_code

@bp.route('/incomes', methods=['GET'])
@jwt_required()
def get_all_incomes_route():
    user_id = get_jwt_identity()
    incomes_data, status_code = get_all_incomes(user_id)
